...
export const createPatientSchema = z.object({
  ...
  age: z.coerce.number({ invalid_type_error: "Age is required" }).int("Age must be a whole number").min(0, "Age must be 0 or more").max(120, "Age seems too high"),
  phone: z.string().min(10, "Valid phone number required"),
...
export const createMedicineSchema = z.object({
  ...
  unitCost: z.number().int("Unit cost must be a whole number (no decimals)").min(0),
  sellingPrice: z.number().int("Selling price must be a whole number (no decimals)").min(0),
  batchNumber: z.string().optional(),
  expiryDate: z.string().optional(),
...
export const updateMedicineSchema = z.object({
  ...
  unitCost: z.number().int("Unit cost must be a whole number (no decimals)").min(0).optional(),
  sellingPrice: z.number().int("Selling price must be a whole number (no decimals)").min(0).optional(),
  batchNumber: z.string().optional(),
  expiryDate: z.string().optional(),
  supplier: z.string().optional(),
  storageCondition: z.string().optional(),
